...
%% upsample GRID & MEDIUM

% upsample along x
medium_upx.sound_speed = zeros(2*Nx, Ny);
medium_upx.density     = zeros(2*Nx, Ny);
for x = 1:Nx
    medium_upx.sound_speed(2*x-1,:) = medium.sound_speed(x,:);
    medium_upx.sound_speed(2*x  ,:) = medium.sound_speed(x,:);
    medium_upx.density(2*x-1,:)     = medium.density(x,:);
    medium_upx.density(2*x  ,:)     = medium.density(x,:);
end

% upsample along y
medium_upx_upy.sound_speed = zeros(2*Nx, 2*Ny);
medium_upx_upy.density     = zeros(2*Nx, 2*Ny);
for y = 1:Ny
    medium_upx_upy.sound_speed(:,2*y-1) = medium_upx.sound_speed(:,y);
    medium_upx_upy.sound_speed(:,2*y  ) = medium_upx.sound_speed(:,y);
    medium_upx_upy.density(:,2*y-1)     = medium_upx.density(:,y);
    medium_upx_upy.density(:,2*y  )     = medium_upx.density(:,y);
end

medium_og = medium;
medium = medium_upx_upy;

% upsample grid
dx = dx/2;                  % grid point spacing in the x direction [m]
dy = dy/2;                  % grid point spacing in the y direction [m]
Nx = Nx*2;                  % number of grid points in the x (row) direction
Ny = Ny*2;                  % number of grid points in the y (column) direction
kgrid = kWaveGrid(Nx, dx, Ny, dy);
...
params.Nt_zero_pad_source   = 100;
params.Nt_t0_correct        = -32;
