...
%     b           - rotation angle of transmitted plane wave in yz plane [rad]
%
% OPTIONAL INPUTS:
...
%     update      - 6 July 2021 (Khoa - corrections for pwUS, 
%                                       rename output & tidy comments &
%                                       explicitly distinquish between
%                                       receive and object k space)
%     last update - 6 July 2021 (Khoa - mod for steered pwUS)
%       
...
% w/c = (kx'^2 + ky'^2 + kz'^2)/(2*kx'*sin(a)*cos(b)+2*ky'*sin(a)*sin(b)+2*kz'*cos(a))    (steered pwUS)
% kx = ky' - w*sin(a)*cos(b)/c              (steered pwUS)
% ky = kz' - w*sin(a)*sin(b)/c              (steered pwUS)
