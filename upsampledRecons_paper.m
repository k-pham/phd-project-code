...
plot(t_array*1e6, squeeze(sensor_data(half_x,half_y,:)))
    title([strtok(file_name,'@') ' x' num2str(half_x) ' y' num2str(half_y)],'Interpreter','None')
    xlabel('time [\mum]')
...
% data = load('D:\PROJECT\code\recon_data\porkBelly3_BK31[CNT]_trimmed_tgc150_interp.mat');
...
data = load('D:\PROJECT\code\recon_data\USIPAPER backups\fly through videos\porkBelly3_BK31[CNT]_trimmed_tgc200_interp.mat');
...
% open figure
figure('Position',[300,300,600,450]) % LYMPH [300,300,600,300]) % PORK 
...
for frame = 1 : num_frames
    ...
    imagesc(x_axis,z_axis,meanIP')
        ...
        text(11.5,10.5,['y = ' sprintf('%0.1f', ypos) ' mm'],'FontSize',14,'FontName','Times New Roman','Color',[1 1 1])     % PORK 11.5,4.5 / 11.5,10.5 % LYMPH 10.5,3.5
        set(gca,'FontSize',14)
