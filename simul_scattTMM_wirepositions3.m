...
xpositions = 1536/12*(1:1:11);
ypositions = 1024/8*(1:1:7);

imgqual.signal_wire  = zeros(length(xpositions),length(ypositions));
imgqual.scatter_mean = zeros(length(xpositions),length(ypositions));
imgqual.scatter_std  = zeros(length(xpositions),length(ypositions));
imgqual.SNR_wire     = zeros(length(xpositions),length(ypositions));
imgqual.resoLat      = zeros(length(xpositions),length(ypositions));
imgqual.resoAxi      = zeros(length(xpositions),length(ypositions));
...
switch simu.params.object_shape
    ...
    case 'wire'
        ...
        imgqual.signal_wire(xidx,yidx)  = signal_wire;
        imgqual.scatter_mean(xidx,yidx) = scatter_mean;
        imgqual.scatter_std(xidx,yidx)  = scatter_std;
        imgqual.SNR_wire(xidx,yidx)     = SNR_wire;
        imgqual.resoLat(xidx,yidx)      = resoLat;
        imgqual.resoAxi(xidx,yidx)      = resoAxi;
...
function [freqT, freqX, sensor_data_fftTX] = fourier_transform_sensor_data(sensor) %#ok<DEFNU>
...
function sensor_data_fftTX = save_or_subtract_2dfft_source_contribution(sensor_data_fftTX, freqT, freqX, simu) %#ok<DEFNU>
    ...
    % else: subtract it from current data
        ...
        % loading background 2dfft data
        load(file_background_2dfft, 'sensor_data_fftTX_background', 'freqT_background');
...
function [theta_new, freqT_new, sensor_data_fftTA] = transforming_2dfft_data_to_aoi(sensor_data_fftTX, freqT, freqX) %#ok<DEFNU>
...
function fig_2dfft = plot_2dfft_sensor_data(freqT, freqX, sensor_data_fftTX) %#ok<DEFNU>
...
function fig_2dfft_aoi = plot_2dfft_aoi_sensor_data(theta_new, freqT_new, sensor_data_fftTA) %#ok<DEFNU>
